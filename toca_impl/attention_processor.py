...
class FluxAttnProcessor2_0:
    ...
    def __call__(
        ...
        if token_cache_obj is None or token_cache_obj.current['type'] == "full":
             # `sample` projections.
            query = attn.to_q(hidden_states)
            key = attn.to_k(hidden_states)
            value = attn.to_v(hidden_states)
            ...
            if token_cache_obj is not None:
                if cache_dic['cache_type'] == "k-norm":
                    img_k_norm = key.norm(dim=-1, p=2).mean(dim=1)
                    # see-toca cache_dic['k-norm'][-1][current['stream']][current['layer']]['img_mlp']
                    cache_dic['k-norm'][-1][current['stream']][current['layer']]['img_mlp'] = img_k_norm
                    if encoder_hidden_states is None:
                        cache_dic['k-norm'][-1][current['stream']][current['layer']]['total'] = img_k_norm
                elif cache_dic['cache_type'] == "v-norm":
                    img_v_norm = value.norm(dim=-1, p=2).mean(dim=1)
                    # see-toca cache_dic['v-norm'][-1][current['stream']][current['layer']]['img_mlp']
                    cache_dic['v-norm'][-1][current['stream']][current['layer']]['img_mlp'] = img_v_norm
                    if encoder_hidden_states is None:
                        cache_dic['v-norm'][-1][current['stream']][current['layer']]['total'] = img_v_norm
            ...
            if encoder_hidden_states is not None:
                ...
                if token_cache_obj is not None:
                    if cache_dic['cache_type'] == "k-norm":
                        # see-toca cache_dic['k-norm'][-1][current['stream']][current['layer']]['txt_mlp']
                        txt_k_norm = encoder_hidden_states_key_proj.norm(dim=-1, p=2).mean(dim=1)
                        cache_dic['k-norm'][-1][current['stream']][current['layer']]['txt_mlp'] = txt_k_norm
                    elif cache_dic['cache_type'] == "v-norm":
                        # see-toca cache_dic['v-norm'][-1][current['stream']][current['layer']]['txt_mlp']
                        txt_v_norm = encoder_hidden_states_value_proj.norm(dim=-1, p=2).mean(dim=1)
            ...
            if token_cache_obj is None:
                ...
                hidden_states, score = dot_product_attention(query, key, value)
                hidden_states = hidden_states.transpose(1, 2).reshape(batch_size, -1, attn.heads * head_dim)
                hidden_states = hidden_states.to(query.dtype)
                if cache_dic['cache_type'] ==  "attention":
                    # see-toca cache_dic['attn_map'][-1][current['stream']][current['layer']]['total']
                    cache_dic['attn_map'][-1][current['stream']][current['layer']]['total'] = score
                # see-toca cache_dic['cache'][-1]['double_stream'][current['layer']]['attn']
                if current['stream'] == "single_stream":
                    token_cache_obj.force_init(hidden_states)
                cache_dic['cache'][-1][current['stream']][current['layer']]['attn'] = hidden_states
                if token_cache_obj is not None and current['type'] == 'full' and encoder_hidden_states is not None:
                    cache_dic['attn_map'][-1][current['stream']][current['layer']]['txt_mlp'] = score[:, : encoder_hidden_states.shape[1]]
                    cache_dic['attn_map'][-1][current['stream']][current['layer']]['img_mlp'] = score[:, encoder_hidden_states.shape[1]: ]
        else:
            inner_dim = attn.to_k.out_features
            head_dim = inner_dim // attn.heads
            query = hidden_states
            hidden_states = cache_dic['cache'][-1][current['stream']][current['layer']]['attn']
